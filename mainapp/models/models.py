...
class Request(models.Model):
    district = models.CharField(
        max_length=15, choices=districts, verbose_name="District - ജില്ല"
    )
    ...
    detailwater = models.CharField(
        max_length=250,
        verbose_name="Details for required water - ആവശ്യമായ വെള്ളത്തിന്റെ വിവരങ്ങള്‍",
        blank=True,
    )
    detailfood = models.CharField(
        max_length=250,
        verbose_name="Details for required food - ആവശ്യമായ ഭക്ഷണത്തിന്റെ വിവരങ്ങള്‍",
        blank=True,
    )
    detailcloth = models.CharField(
        max_length=250,
        verbose_name="Details for required clothing - ആവശ്യമായ വസ്ത്രത്തിന്റെ വിവരങ്ങള്‍",
        blank=True,
    )
    detailmed = models.CharField(
        max_length=250,
        verbose_name="Details for required medicine - ആവശ്യമായ മരുന്നിന്റെ  വിവരങ്ങള്‍",
        blank=True,
    )
    ...
    def __str__(self):
        return (
            "#" + str(self.id) + " " + self.get_district_display() + " " + self.location
        )
...
class NGO(models.Model):
    ...
    organisation_type = models.CharField(
        max_length=250, verbose_name="Type of Organization"
    )
    organisation_address = models.TextField(
        default="", verbose_name="Address of Organization"
    )
    ...
    website_url = models.CharField(
        max_length=300, verbose_name="Enter your website link", default=""
    )
    is_spoc = models.BooleanField(default=False, verbose_name="Is point of contact")
    joined = models.DateTimeField(auto_now_add=True)
...
class Contributor(models.Model):
    ...
    address = models.TextField(verbose_name="Address - വിലാസം")
    contrib_details = models.TextField(
        verbose_name="Details of contribution Eg: 10 shirts", default=""
    )
    status = models.CharField(
        max_length=10, choices=contrib_status_types, default="new"
    )
    contribution_type = models.CharField(
        max_length=3, choices=contribution_types, default="oth"
    )
...
class DistrictNeed(models.Model):
    ...
    cnandpts = models.TextField(
        verbose_name="Contacts and collection points"
...
class RescueCamp(models.Model):
    ...
    status = models.CharField(
        max_length=10, choices=relief_camp_status, default="active",
    )
...
class PrivateRescueCamp(models.Model):
    ...
    ward_name = models.CharField(
        max_length=150, null=True, blank=True, verbose_name="Ward - വാർഡ്"
    )
    contacts = models.TextField(
        verbose_name="Phone Numbers - ഫോൺ നമ്പറുകൾ", blank=True, null=True
    )
...
class Person(models.Model):
    name = models.CharField(
        max_length=51, blank=False, null=False, verbose_name="Name - പേര്"
    )
    phone = models.CharField(
        max_length=14, null=True, blank=True, verbose_name="Mobile - മൊബൈൽ"
    )
    age = models.IntegerField(null=True, blank=True, verbose_name="Age - പ്രായം")
    gender = models.IntegerField(
        choices=gender, verbose_name="Gender - ലിംഗം", null=True, blank=True
    )
    ...
    notes = models.TextField(
        max_length=500, null=True, blank=True, verbose_name="Notes - കുറിപ്പുകൾ"
    )
    ...
    checkin_date = models.DateField(
        null=True, blank=True, verbose_name="Check-in Date - ചെക്ക്-ഇൻ തീയതി"
    )
    checkout_date = models.DateField(
        null=True, blank=True, verbose_name="Check-out Date - ചെക്ക്-ഔട്ട് തീയതി"
    )

    status = models.CharField(
        blank=True, null=True, max_length=15, choices=person_status, default=None,
    )
...
class DataCollection(models.Model):
    ...
    document_name = models.CharField(
        max_length=255, null=True, blank=True, verbose_name="Document name"
    )
...
class RequestUpdate(models.Model):
    ...
    updater_name = models.CharField(
        max_length=100, verbose_name="Name of person or group updating", blank=False
    )
...
class CollectionCenter(models.Model):
    ...
    name = models.CharField(
        max_length=100, blank=False, null=False, verbose_name="Name - പേര്"
    )
    address = models.TextField(verbose_name="Address - വിലാസം")
    contacts = models.CharField(
        max_length=250, null=True, blank=True, verbose_name="Contacts - മൊബൈൽ"
    )
    ...
    is_inside_kerala = models.BooleanField(
        default=True, verbose_name="Center inside kerala? - കേന്ദ്രം കേരളത്തിലാണോ"
    )
    city = models.CharField(
        null=True, blank=True, max_length=150, verbose_name="City - നഗരം"
    )
    added_at = models.DateTimeField(auto_now_add=True)
    ...
    def full_clean(self, *args, **kwargs):
        ...
        if len(flds - person_flds) == 0:
            ...
            raise ValidationError(
                "Invalid CSV headers found: " + str(flds - person_flds)
            )
