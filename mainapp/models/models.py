...
status_types = (("new", "New"), ("pro", "In progess"), ("sup", "Supplied"))
...
person_status = (("new", "New"), ("checked_out", "Checked Out"), ("closed", "Closed"))
...
class Request(models.Model):
    district = models.CharField(max_length=15, choices=districts, verbose_name="District - ജില്ല")
    ...
    requestee_phone = models.CharField(
        max_length=14, verbose_name="Requestee Phone - അപേക്ഷകന്റെ ഫോണ്‍ നമ്പര്‍", validators=[phone_number_regex],
    )
    ...
    detailwater = models.CharField(
        max_length=250, verbose_name="Details for required water - ആവശ്യമായ വെള്ളത്തിന്റെ വിവരങ്ങള്‍", blank=True,
    )
    detailfood = models.CharField(
        max_length=250, verbose_name="Details for required food - ആവശ്യമായ ഭക്ഷണത്തിന്റെ വിവരങ്ങള്‍", blank=True,
    )
    detailcloth = models.CharField(
        max_length=250, verbose_name="Details for required clothing - ആവശ്യമായ വസ്ത്രത്തിന്റെ വിവരങ്ങള്‍", blank=True,
    )
    detailmed = models.CharField(
        max_length=250, verbose_name="Details for required medicine - ആവശ്യമായ മരുന്നിന്റെ  വിവരങ്ങള്‍", blank=True,
    )
    ...
    status = models.CharField(max_length=10, choices=status_types, default="new")
...
class VolunteerGroup(models.Model):
    ...
    def __str__(self):
        return (
            ...
            + str(self.group_name)
            + " || Volunteer Count "
            + str(Volunteer.objects.all().filter(groups__in=[self]).count())
        )
...
class Volunteer(models.Model):
    ...
    area = models.CharField(max_length=15, choices=vol_categories, verbose_name="Area of volunteering - സന്നദ്ധസേവനം",)
...
class NGO(models.Model):
    district = models.CharField(max_length=15, choices=districts,)
...
class Contributor(models.Model):
    ...
    status = models.CharField(max_length=10, choices=contrib_status_types, default="new")
    contribution_type = models.CharField(max_length=3, choices=contribution_types, default="oth")
...
class RescueCamp(models.Model):
    ...
    facilities_available = models.TextField(
        blank=True, null=True, verbose_name="Facilities Available (light, kitchen, toilets etc.) - ലഭ്യമായ സൗകര്യങ്ങൾ",
    )
    ...
    status = models.CharField(max_length=10, choices=relief_camp_status, default="active",)
...
class PrivateRescueCamp(models.Model):
    ...
    lsg_name = models.CharField(
        max_length=150, null=True, blank=True, verbose_name="LSG Name - സ്വയംഭരണ സ്ഥാപനത്തിന്റെ പേര്",
    )
...
class Person(models.Model):
    ...
    gender = models.IntegerField(choices=gender, verbose_name="Gender - ലിംഗം", null=True, blank=True)
    ...
    camped_at = models.ForeignKey(
        RescueCamp, models.CASCADE, blank=False, null=False, verbose_name="Camp Name - ക്യാമ്പിന്റെ പേര്",
    )
    ...
    status = models.CharField(blank=True, null=True, max_length=15, choices=person_status, default=None,)
    ...
    @property
    def sex(self):
        return {0: "Male", 1: "Female", 2: "Others"}.get(self.gender, "Unknown")
...
class DataCollection(models.Model):
    ...
    document_name = models.CharField(max_length=255, null=True, blank=True, verbose_name="Document name")
...
class RequestUpdate(models.Model):
    ...
    status = models.CharField(max_length=10, choices=volunteer_update_status_types)

    other_status = models.CharField(
        max_length=255, verbose_name="Please specify other status", default="", blank=True,
    )
    ...
    updater_phone = models.CharField(
        max_length=14, verbose_name="Phone number of person or group updating", validators=[phone_number_regex],
    )
...
class CollectionCenter(models.Model):
    ...
    type_of_materials_collecting = models.TextField(
        verbose_name="Type of materials collecting - ശേഖരിക്കുന്ന വസ്തുക്കൾ ", null=True, blank=True,
    )
    district = models.CharField(
        max_length=15, choices=districts, verbose_name="Ceter District - ജില്ല", null=True, blank=True,
    )
    lsg_type = models.SmallIntegerField(
        choices=lsg_types, verbose_name="LSG Type - തദ്ദേശ സ്വയംഭരണ സ്ഥാപനം", null=True, blank=True,
    )
    lsg_name = models.CharField(
        max_length=150, null=True, blank=True, verbose_name="LSG Name - സ്വയംഭരണ സ്ഥാപനത്തിന്റെ പേര്",
    )
...
class CsvBulkUpload(models.Model):
    ...
    failure_reason = models.CharField(
        max_length=150, default="", blank=True, verbose_name="Reason of failure, if failed",
    )
...
class Hospital(models.Model):
    district = models.CharField(
        max_length=15, choices=districts, verbose_name="District - ജില്ല", null=True, blank=True,
    )
